...
def gen_pos(disp,cell_x=3.55648,cell_y=3.55648,cell_z=3.55648):
  ...
  cell_y = float(cell_y)
  cell_z = float(cell_z)
  atom_rel_pos = [[0,0,0],[2.0/4,2.0/4,0],[3.0/4,1.0/4,1.0/4],[1.0/4,3.0/4,1.0/4],[1.0/4,1.0/4,3.0/4],[3.0/4,3.0/4,3.0/4],[2.0/4,0,2.0/4],[0,2.0/4,2.0/4]]
  atom_abs_pos = []
  for count in range(len(atom_rel_pos)):
    ...
    elif ((count == 4) | (count == 5) | (count == 6) | (count == 7)):
      atom_abs_pos.append([atom_rel_pos[count][0]*cell_x,atom_rel_pos[count][1]*cell_y,(atom_rel_pos[count][2]-disp)*cell_z])
