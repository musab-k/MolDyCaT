...
def gen_pos(disp,cell_x=3.55648,cell_y=3.55648,cell_z=3.55648):
  ...
  cell_y = float(cell_y)
  cell_z = float(cell_z)
  atom_rel_pos = [[0,1.0/4,0],[2.0/4,3.0/4,0],[3.0/4,2.0/4,1.0/4],[1.0/4,0,1.0/4],[0,3.0/4,2.0/4],[2.0/4,1.0/4,2.0/4],[1.0/4,2.0/4,3.0/4],[3.0/4,0,3.0/4]]
  atom_abs_pos = []
  for count in range(len(atom_rel_pos)):
    ...
    elif ((count == 2) | (count == 3) | (count == 4) | (count == 5)):
      atom_abs_pos.append([(atom_rel_pos[count][0]-disp)*cell_x,(atom_rel_pos[count][1]-disp)*cell_y,atom_rel_pos[count][2]*cell_z])
    '''
